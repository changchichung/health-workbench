...
// 關聯資料表；順序即刪除順序（medications 引用 encounters，MUST 先刪）
...
// apple 兩表的去重鍵比對條件（鏡像 schema 的 UNIQUE 定義；
// apple_records 的 UNIQUE INDEX 用 COALESCE，apple_workouts 用
// 一般欄位（NULL 彼此相異），MUST 分別鏡像，不得互換）
const APPLE_MATCH = {
...
// apple 表：來源 doc 中與目標成員同去重鍵的列
const appleDupSelect = (t) =>
  `SELECT s.id FROM ${t} s WHERE s.doc_id=? AND EXISTS(SELECT 1 FROM ${t} x`
  + ` WHERE x.profile_id=? AND ${APPLE_MATCH[t]})`;
...
async function mergeCounts(driver, docId, targetProfileId) {
  ...
  for (const t of ["apple_records", "apple_workouts"]) {
    const [{ c }] = await driver.select(
      `SELECT count(*) c FROM (${appleDupSelect(t)})`, [docId, targetProfileId]);
    perTable[t] = c;
  }
...
export async function reattributeSourceDocument(driver, docId, targetProfileId) {
  return driver.transaction(async (tx) => {
    ...
    // apple 表：同去重鍵刪來源列、其餘改掛
    for (const t of ["apple_records", "apple_workouts"]) {
      const dups = await tx.select(appleDupSelect(t), [docId, target.id]);
      if (dups.length) {
        const marks = dups.map(() => "?").join(",");
        await tx.execute(
          `DELETE FROM ${t} WHERE id IN (${marks})`, dups.map(x => x.id));
